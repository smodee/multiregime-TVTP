...
test_that("off-diagonal transform uses logit for transition params", {
  par <- c(-1, 1, 0.5, 0.6, 0.2, 0.1)
  par <- set_parameter_attributes(par, K = 2, model_type = "constant",
  ...
  # transition params logit-transformed
  expect_equal(par_t[5], log(0.2 / 0.8), tolerance = 1e-12)
  expect_equal(par_t[6], log(0.1 / 0.9), tolerance = 1e-12)
})
...
test_that("off-diagonal transform round-trip for TVP model", {
  # mu1, mu2, sigma2_1, sigma2_2, p12, p21, A1, A2
  par <- c(-1, 1, 0.5, 0.8, 0.2, 0.15, 0.1, -0.1)
...
test_that("off-diagonal transform round-trip for GAS model", {
  ...
  par <- c(-1, 1, 0.5, 0.8, 0.2, 0.15, 0.1, -0.1, 0.5, 0.6)
...
test_that("off-diagonal transform rejects out-of-bound probabilities", {
  par <- c(-1, 1, 0.5, 0.6, 0.0, 0.5)  # p12 = 0 is invalid
  par <- set_parameter_attributes(par, K = 2, model_type = "constant",
                                  diag_probs = FALSE, equal_variances = FALSE)
  expect_error(transform_parameters(par), "strictly between 0 and 1")
})
